...
import ScheduleManagement from "@/components/Magazine/ScheduleManagement/ScheduleManagement"

export const Route = createFileRoute("/_auth/_dashboard/mag/manage-schedule/$advertiserId")({
    component: () => {
        // Get agent ID from URL params or however you're passing it
        const auth = useAuth()
        const { advertiserId } = Route.useParams()
        ...
        return (
            <div className="grid grid-rows-[6rem_2rem_1fr_2rem] gap-4 h-full">
                <ScheduleManagement advertiserId={advertiserId} />
            </div>
        )
    },
})