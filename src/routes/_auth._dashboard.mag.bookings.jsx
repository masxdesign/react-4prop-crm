...
export const Route = createFileRoute('/_auth/_dashboard/mag/bookings')({
  ...
  beforeLoad: ({ context, search }) => {
    ...
    const queryOptions = {
      queryKey: ['bookings', isAdvertiser ? 'advertiser' : 'agent', userId, status, page, pageSize],
      queryFn: () => {
        if (isAdvertiser) {
          return fetchAdvertiserBookings(userId, { status, page, pageSize });
        } else {
          return fetchAgentBookings(userId, { status, page, pageSize });
        }
      },
      enabled: !!userId,
    };
