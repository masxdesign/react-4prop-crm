...
/**
 * AdvertiserSelectionTable Component (for Booking History)
 *
 * Displays a searchable, paginated list of advertisers for admin selection.
 * All state (page, search, sorting) is synced with URL search parameters.
 * Clicking a row navigates to that advertiser's booking history page.
 */
const AdvertiserSelectionTable = () => {
  ...
  // Handle row click - navigate to advertiser booking history page
  const handleRowClick = (advertiser) => {
    navigate({ to: `/crm/booking-history/advertiser/${advertiser.id}` });
  };
